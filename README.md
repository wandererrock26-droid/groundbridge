...
авто-откатом. Если новый адрес отрезал станцию от панели, она через
полторы минуты сама вернёт прежний. Встроенная плата всегда `eth0`,
USB-адаптер всегда `eth1` — это правило udev, а не порядок включения, и
статика больше не «уезжает» на чужую железку. Настройки переживают
обновление станции — потерять адрес машины в поле нельзя.
См. [docs/network.md](docs/network.md).

**RTSP-камера идёт мимо станции.** Для внешней RTSP-камеры добавлен
**проброс без обработки**: поток тянет сам MediaMTX, плата в видеотракте не
участвует вовсе. Разрешение, частота кадров и кодек остаются камерными,
задержки и нагрузки станция не добавляет — на апробации именно лишнее звено
заставляло картинку «троить». Пропала камера — автоматически показывается
заглушка, вернулась — снова проброс. См. [docs/video.md](docs/video.md).
...
десять секунд. Живой поток — уже доказательство, что камера на месте.
